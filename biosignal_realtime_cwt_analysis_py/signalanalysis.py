...
def signal_analysis(data_q, data_s, shutdown_e, quasi_freq_filter):
    ...
    try:
        ...
        while not shutdown_e.is_set():
            ...
            cwt_data = get_cwt(pool, data_payload.cwt_data, cwt_dt,
                               data_payload.cwt_data_length.value + SMOOTH)
            #

            #
            if first_time:
                first_time = False
                data_s.put(data_payload.cwt_data_length, PUTBLOCK, PUTTIMEOUT)
            #
